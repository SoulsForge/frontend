import { DownloadIcon, EditIcon, HeartIcon } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  ...
  return (
    <article className='flex flex-col gap-4'>
      <section className='mt-8 flex w-full flex-row items-start justify-around'>
        ...
        {!isOwner && (
          <Button variant='ghost' onClick={handleFavorite} className='ml-2'>
            <HeartIcon
              className={cn('size-full', isFavorite && 'fill-rose-600')}
            />
          </Button>
        )}
      </section>
